-- Enhanced Security Fix for Contact Submissions (Corrected)
...
-- 7. Add additional constraint to prevent malformed data
ALTER TABLE public.contact_submissions 
ADD CONSTRAINT IF NOT EXISTS contact_email_format_check 
CHECK (email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$');

-- 8. Create secure wrapper function that logs all contact data access
...
BEGIN
  ...
  IF NOT public.validate_admin_contact_access() THEN
    RAISE EXCEPTION 'Access denied. Enhanced admin validation failed for contact data access.';
  END IF;
  
  -- Log the data access attempt
  PERFORM public.log_security_event(
    'contact_data_bulk_access',
    'high',
    jsonb_build_object(
      'admin_user_id', auth.uid(),
      'timestamp', now(),
      'function_called', 'get_contact_submissions_with_audit'
    )
  );
  
  -- Return decrypted contact data using existing secure function
  RETURN QUERY
  SELECT * FROM public.get_contact_submissions_secure();
END;
$$;